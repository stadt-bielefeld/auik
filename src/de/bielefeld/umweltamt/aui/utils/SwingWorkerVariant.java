 ...
 * GlassPane class to allow users to execute timeconsuming task on a separate
 * thread The GlassPane addition can prevent users from executing another
 * SwingWorker task while one SwingWorker task is already executing<br>
 * <br>
...
//    private Thread thread;
    ...
    public SwingWorkerVariant(Component aComponent) {
        ...
        Thread t = new Thread(
            Thread.currentThread().getThreadGroup(), doConstruct);
        ...
        try {
            ...
            if (e.getCause() instanceof HibernateException) {
                DatabaseManager.getInstance().handleDBException(
                    e, "SwingWorker.doNonUILogic", fatalError);
            } else {
                throw new RuntimeException(
                    "SwingWorker error in doNonUILogic", e);
            }
        }
    }
     ...
     * @throws java.lang.RuntimeException thrown if there are any problems
     *             executing the ui update logic
     */
    ...
    protected void finished() {
        try {
            ...
            throw new RuntimeException(
                "SwingWorker error in doUIUpdateLogic", e);
        } finally {
