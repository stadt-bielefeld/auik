...
/**
 * Das Modul um die Kläranlagentabelle zu bearbeiten.
 * @author Gerd Genuit
 */
public class EditorKlaeranlagen extends AbstractModul {
	/** Logging */
    ...
    private EditorKlaeranlageModel tmodel;
    ...
    public JPanel getPanel() {
        if (panel == null) {
            ...
            submitButton = new JButton("Alle Kläranlagen anzeigen");
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (row != -1 && getResultTable().getEditingRow() == -1) {
                        ...
                        if (GUIManager.getInstance().showQuestion(
                            "Soll die Einheit " + ka.getAnlage()
                                + " wirklich gelöscht werden?\n",
                            "Löschen bestätigen")) {
                            if (EditorKlaeranlagen.this.tmodel
                                .removeRow(row)) {
                            	EditorKlaeranlagen.this.frame.changeStatus(
                                    "Einheit gelöscht.",
                                    HauptFrame.SUCCESS_COLOR);
                                log.debug("Objektart " + ka.getAnlage()
                                    + " wurde gelöscht!");
                            } else {
                            	EditorKlaeranlagen.this.frame.changeStatus(
