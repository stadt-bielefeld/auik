...
/**
 * Das Modul um die Einheitentabelle zu bearbeiten.
 * @author Gerd Genuit
 */
public class EditorSachbearbeiter extends AbstractModul {
	/** Logging */
    ...
    public JPanel getPanel() {
        if (panel == null) {
            ...
            submitButton = new JButton("Alle Sachbearbeiter anzeigen");
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (row != -1 && getResultTable().getEditingRow() == -1) {
                        ...
                        if (GUIManager.getInstance().showQuestion(
                            "Soll die Einheit " + sachbearbeiter.getName()
                                + " wirklich gelöscht werden?\n",
                            "Löschen bestätigen")) {
                            if (EditorSachbearbeiter.this.tmodel
                                .removeRow(row)) {
                            	EditorSachbearbeiter.this.frame.changeStatus(
                                    "Einheit gelöscht.",
                                    HauptFrame.SUCCESS_COLOR);
                                log.debug("Objektart " + sachbearbeiter.getName()
                                    + " wurde gelöscht!");
                            } else {
                            	EditorSachbearbeiter.this.frame.changeStatus(
