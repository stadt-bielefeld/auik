...
import de.bielefeld.umweltamt.aui.module.common.tablemodels.EditorObjektartenModel;
import de.bielefeld.umweltamt.aui.utils.AuikLogger;
import de.bielefeld.umweltamt.aui.utils.SwingWorkerVariant;
import de.bielefeld.umweltamt.aui.utils.tablemodelbase.ListTableModel;

/**
 * Das Modul um die Objektartentabelle zu bearbeiten.
 * @author Gerd Genuit
 */
public class EditorObjektarten extends AbstractModul {
	/** Logging */
    ...
    private EditorObjektartenModel tmodel;
    ...
    public JPanel getPanel() {
        if (panel == null) {
            ...
            submitButton = new JButton("Alle Objektarten anzeigen");
            ...
            this.resultLoeschAction = new AbstractAction("Löschen") {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (row != -1 && getResultTable().getEditingRow() == -1) {
                    	BasisObjektarten art = EditorObjektarten.this.tmodel
                            .getRow(row);

                        if (GUIManager.getInstance().showQuestion(
                            "Soll die Objektart " + art.getObjektart()
                                + " wirklich gelöscht werden?\n",
                            "Löschen bestätigen")) {
                            if (EditorObjektarten.this.tmodel
                                .removeRow(row)) {
                            	EditorObjektarten.this.frame.changeStatus(
                                    "Objektart gelöscht.",
                                    HauptFrame.SUCCESS_COLOR);
                                log.debug("Objektart " + art.getObjektart()
                                    + " wurde gelöscht!");
                            } else {
                            	EditorObjektarten.this.frame.changeStatus(
