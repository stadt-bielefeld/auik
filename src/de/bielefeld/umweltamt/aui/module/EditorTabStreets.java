...
public class EditorTabStreets extends AbstractModul
{
	...
	private EditorTabStreetsModel tmodel;
	...
	public JPanel getPanel()
	{
		if (panel == null)
		{
			...
			submitButton = new JButton("Alle Standorte anzeigen");
	...
	public ListTableModel<TabStreets> getTableModel()
	{
		if (tmodel == null)
		{
			tmodel = new EditorTabStreetsModel();
		}
		return tmodel;
	}
				...
				public void actionPerformed(ActionEvent e)
				{
					...
					if (row != -1 && getResultTable().getEditingRow() == -1)
					{
						...
						if (GUIManager.getInstance().showQuestion(
																	"Soll die Straße " + strasse.getName()
																			+ " und alle seine Fachdaten "
																			+ "wirklich gelöscht werden?\n",
																	"Löschen bestätigen"))
						{
							if (EditorTabStreets.this.tmodel
									.removeRow(row))
							{
								EditorTabStreets.this.frame.changeStatus(
																		"Objekt gelöscht.",
																		HauptFrame.SUCCESS_COLOR);
								log.debug("Objekt " + strasse.getName()
										+ " wurde gelöscht!");
							}
							else
							{
