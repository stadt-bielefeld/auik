...
import de.bielefeld.umweltamt.aui.module.common.tablemodels.EditorEntsorgerModel;
import de.bielefeld.umweltamt.aui.utils.AuikLogger;
import de.bielefeld.umweltamt.aui.utils.SwingWorkerVariant;
import de.bielefeld.umweltamt.aui.utils.tablemodelbase.ListTableModel;

/**
 * Das Modul um die Entsorgertabelle zu bearbeiten.
 * @author Gerd Genuit
 */
public class EditorEntsorger extends AbstractModul {
	/** Logging */
    ...
    private EditorEntsorgerModel tmodel;
    ...
    public JPanel getPanel() {
        if (panel == null) {
            ...
            submitButton = new JButton("Alle Entsorger anzeigen");
    ...
    private Action getResultLoeschAction() {
        if (this.resultLoeschAction == null) {
            this.resultLoeschAction = new AbstractAction("Löschen") {
				...
				private static final long serialVersionUID = -7386897959190537404L;

				@Override
                public void actionPerformed(ActionEvent e) {
                    int row = getResultTable().getSelectedRow();
                    if (row != -1 && getResultTable().getEditingRow() == -1) {
                    	AnhEntsorger entsorger = EditorEntsorger.this.tmodel
                            .getRow(row);

                        if (GUIManager.getInstance().showQuestion(
                            "Soll die Einheit " + entsorger.getEntsorger()
                                + " wirklich gelöscht werden?\n",
                            "Löschen bestätigen")) {
                            if (EditorEntsorger.this.tmodel
                                .removeRow(row)) {
                            	EditorEntsorger.this.frame.changeStatus(
                                    "Einheit gelöscht.",
                                    HauptFrame.SUCCESS_COLOR);
                                log.debug("Objektart " + entsorger.getEntsorger()
                                    + " wurde gelöscht!");
                            } else {
                            	EditorEntsorger.this.frame.changeStatus(
                                    "Konnte die Objektart nicht löschen!",
