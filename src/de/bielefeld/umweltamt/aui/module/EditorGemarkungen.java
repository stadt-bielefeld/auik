 ...
 * @author Gerd Genuit
 */
public class EditorGemarkungen extends AbstractModul {
	/** Logging */
    ...
    public JPanel getPanel() {
        if (panel == null) {
            ...
            submitButton = new JButton("Alle Gemarkungen anzeigen");
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (row != -1 && getResultTable().getEditingRow() == -1) {
                        ...
                        if (GUIManager.getInstance().showQuestion(
                            "Soll die Einheit " + gemarkung.getGemarkung()
                                + " wirklich gelöscht werden?\n",
                            "Löschen bestätigen")) {
                            if (EditorGemarkungen.this.tmodel
                                .removeRow(row)) {
                            	EditorGemarkungen.this.frame.changeStatus(
                                    "Einheit gelöscht.",
                                    HauptFrame.SUCCESS_COLOR);
                                log.debug("Objektart " + gemarkung.getGemarkung()
                                    + " wurde gelöscht!");
                            } else {
                            	EditorGemarkungen.this.frame.changeStatus(
