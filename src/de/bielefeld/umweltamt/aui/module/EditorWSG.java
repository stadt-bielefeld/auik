...
import de.bielefeld.umweltamt.aui.module.common.tablemodels.EditorWSGModel;
import de.bielefeld.umweltamt.aui.utils.AuikLogger;
import de.bielefeld.umweltamt.aui.utils.SwingWorkerVariant;
import de.bielefeld.umweltamt.aui.utils.tablemodelbase.ListTableModel;

/**
 * Das Modul um die Wasserschutzgebietetabelle zu bearbeiten.
 * @author Gerd Genuit
 */
public class EditorWSG extends AbstractModul {
	/** Logging */
    ...
    public JPanel getPanel() {
        if (panel == null) {
            ...
            submitButton = new JButton("Alle Wasserschutzgebiete anzeigen");
            ...
            this.resultLoeschAction = new AbstractAction("Löschen") {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (row != -1 && getResultTable().getEditingRow() == -1) {
                    	VawsWassereinzugsgebiete wsg = EditorWSG.this.tmodel
                            .getRow(row);

                        if (GUIManager.getInstance().showQuestion(
                            "Soll das Wasserschutzgebiet " + wsg.getEzgbname()
                                + " wirklich gelöscht werden?\n",
                            "Löschen bestätigen")) {
                            if (EditorWSG.this.tmodel
                                .removeRow(row)) {
                            	EditorWSG.this.frame.changeStatus(
                                    "Einheit gelöscht.",
                                    HauptFrame.SUCCESS_COLOR);
                                log.debug("Objektart " + wsg.getEzgbname()
                                    + " wurde gelöscht!");
                            } else {
                            	EditorWSG.this.frame.changeStatus(
