/*
 * Datei:
 * $Id: ObjektProbepktAuswPanel.java,v 1.1 2009-09-21 11:14:51 u633d Exp $
 * 
...
public class ObjektProbepktAuswPanel extends JPanel {
	...
	private class AuswertungsDialog extends JDialog {
		...
		private class ExportTableModel extends AbstractTableModel {
			...
			public int getColumnCount() {
				return col1.getSeriesCount()
						+ ((col2 != null) ? col2.getSeriesCount() : 0) + 1;// 2;
			}
			...
			public Object getValueAt(int rowIndex, int columnIndex) {
				...
				/*
				 * if (columnIndex == 0) { APosDataItem item; String probe =
				 * null; for (int i = 0; i < col1.getSeriesCount(); i++) { item
				 * = (APosDataItem) col1.getSeries(i).getDataItem(min); if (item
				 * != null) { probe =
				 * item.getAnalysePosition().getAtlProbenahmen().getKennummer();
				 * break; } }
				 * 
				 * if (probe == null && col2 != null) { for (int i = 0; i <
				 * col2.getSeriesCount(); i++) { item = (APosDataItem)
				 * col2.getSeries(i).getDataItem(min); if (item != null) { probe
				 * =
				 * item.getAnalysePosition().getAtlProbenahmen().getKennummer();
				 * break; } } } tmp = probe; } else
				 */if (columnIndex == 0) {
					...
					if (seriesIndex < col1.getSeriesCount()) {
						item = (APosDataItem) col1.getSeries(seriesIndex)
								.getDataItem(min);
					} else if (col2 != null) {
						item = (APosDataItem) col2.getSeries(series2Index)
								.getDataItem(min);
					}
					if (item != null) {
			...
			public String getColumnName(int column) {
				...
				/*
				 * if (column == 0) { tmp = "Probe"; } else
				 */if (column == 0) {
					tmp = "Datum";
				} else {
					if (seriesIndex < col1.getSeriesCount()) {
						tmp = col1.getSeriesName(seriesIndex)
								+ ", "
								+ col1.getSeries(seriesIndex)
										.getRangeDescription();
					} else if (col2 != null) {
						tmp = col2.getSeriesName(series2Index)
								+ ", "
								+ col2.getSeries(series2Index)
										.getRangeDescription();
					}
				}
		...
		private JComponent createTabellenPanel() {
			exportTable = new JTable(new ExportTableModel(leftDataset,
					rightDataset));
			exportTable.setBorder(BorderFactory
		...
		public void saveTabelle() {
			...
			if (exportDatei != null) {
				...
				if (ext == null) {
					...
					exportDatei = new File(exportDatei.getParent(), exportDatei
							.getName()
							+ newExt);
				}
				...
				if (doIt) {
					AUIKataster.debugOutput("Speichere nach '"
							+ exportDatei.getName() + "' (Ext: '" + ext
							+ "') in '" + exportDatei.getParent() + "' !");
					if (AuikUtils
							.exportTableDataToCVS(exportTable, exportDatei)) {
						AUIKataster.debugOutput("Speichern erfolgreich!");
					} else {
						AUIKataster.debugOutput("Fehler beim Speichern!");
						owner.showErrorMessage("Beim Speichern der Datei '"
	 ...
	 */
	public ObjektProbepktAuswPanel(ObjektBearbeiten hauptModul) {
		name = "Auswertung";
		this.hauptModul = hauptModul;
		{
			...
			FormLayout gesamtLayout = new FormLayout("pref, 5dlu, "
					+ spaltenTeil + ", 10dlu:g(0.2), " + spaltenTeil,
			...
			builder.addSeparator("Zeitraum", cc.xyw(1, 1, 3));
			builder.add(new JLabel("Von:"), cc.xy(1, 3, "r,d"),
					getVonDateChooser(), cc2.xy(3, 3, "l,d"));
			builder.add(new JLabel("Bis:"), cc.xy(1, 5, "r,d"),
					getBisDateChooser(), cc2.xy(3, 5, "l,d"));
	...
	public void showResultOneAxis(final String axis) {
		SwingWorkerVariant worker = new SwingWorkerVariant(getSubmitButton()) {
			...
			protected void doUIUpdateLogic() throws RuntimeException {
				if (dataSet1.getSeriesCount() > 0) {
					hauptModul.getFrame().clearStatus();
					AuswertungsDialog dialog = new AuswertungsDialog(
							getArtBox().getSelectedItem().toString(), dataSet1,
							null, hauptModul.getFrame());
		...
		hauptModul.getFrame().changeStatus("Bereite Auswertung vor...");
		worker.start();
	}
	...
	public void showResultDualAxis() {
		SwingWorkerVariant worker = new SwingWorkerVariant(getSubmitButton()) {
			...
			protected void doNonUILogic() throws RuntimeException {
				...
				seriesCount = dataSet1.getSeriesCount()
						+ dataSet2.getSeriesCount();
			}
	...
	private TimeSeriesCollection createDataset(String axis) {
		...
		if (getHeepenCheck().isSelected()) {
			createSeries(art, AtlKlaeranlagen
					.getKlaeranlage(AtlKlaeranlagen.HEEPEN), einheit,
					paramList, analyseVon, vonDate, bisDate, col);
		}
		if (getBrakeCheck().isSelected()) {
			createSeries(art, AtlKlaeranlagen
					.getKlaeranlage(AtlKlaeranlagen.BRAKE), einheit, paramList,
					analyseVon, vonDate, bisDate, col);
		}
		if (getSennestCheck().isSelected()) {
			createSeries(art, AtlKlaeranlagen
					.getKlaeranlage(AtlKlaeranlagen.SENNESTADT), einheit,
					paramList, analyseVon, vonDate, bisDate, col);
		}
		if (getOlutterCheck().isSelected()) {
			createSeries(art, AtlKlaeranlagen
					.getKlaeranlage(AtlKlaeranlagen.OBERE_LUTTER), einheit,
					paramList, analyseVon, vonDate, bisDate, col);
		}
		if (getVerlCheck().isSelected()) {
			createSeries(art, AtlKlaeranlagen
					.getKlaeranlage(AtlKlaeranlagen.VERL_SENDE), einheit,
					paramList, analyseVon, vonDate, bisDate, col);
		}
	...
	private JPanel getParameterPanel() {
		if (parameterPanel == null) {
			...
			builder.add(new JScrollPane(lList), cc.xywh(1, 3, 1, 15,
					"fill, fill"));
			builder.add(new JScrollPane(rList), cc.xywh(9, 3, 1, 15,
					"fill, fill"));

			builder.add(createRLButton(true, AtlParameter.CADMIUM_ID), cc.xy(3,
					3));
			builder.add(new JLabel("Cadmium (Cd)", JLabel.CENTER), cc.xy(5, 3,
					"f,d"));
			builder.add(createRLButton(false, AtlParameter.CADMIUM_ID), cc.xy(
					7, 3));

			builder.add(createRLButton(true, AtlParameter.CHROM_ID), cc
					.xy(3, 5));
			builder.add(new JLabel("Chrom (Cr)", JLabel.CENTER), cc.xy(5, 5,
					"f,d"));
			builder.add(createRLButton(false, AtlParameter.CHROM_ID), cc.xy(7,
					5));

			builder.add(createRLButton(true, AtlParameter.KUPFER_ID), cc.xy(3,
					7));
			builder.add(new JLabel("Kupfer (Cu)", JLabel.CENTER), cc.xy(5, 7,
					"f,d"));
			builder.add(createRLButton(false, AtlParameter.KUPFER_ID), cc.xy(7,
					7));

			builder.add(createRLButton(true, AtlParameter.QUECKSILBER_ID), cc
					.xy(3, 9));
			builder.add(new JLabel("Quecksilber (Hg)", JLabel.CENTER), cc.xy(5,
					9, "f,d"));
			builder.add(createRLButton(false, AtlParameter.QUECKSILBER_ID), cc
					.xy(7, 9));

			builder.add(createRLButton(true, AtlParameter.NICKEL_ID), cc.xy(3,
					11));
			builder.add(new JLabel("Nickel (Ni)", JLabel.CENTER), cc.xy(5, 11,
					"f,d"));
			builder.add(createRLButton(false, AtlParameter.NICKEL_ID), cc.xy(7,
					11));

			builder.add(createRLButton(true, AtlParameter.BLEI_ID), cc
					.xy(3, 13));
			builder.add(new JLabel("Blei (Pb)", JLabel.CENTER), cc.xy(5, 13,
					"f,d"));
			builder.add(createRLButton(false, AtlParameter.BLEI_ID), cc.xy(7,
					13));

			builder.add(createRLButton(true, AtlParameter.ZINK_ID), cc
					.xy(3, 15));
			builder.add(new JLabel("Zink (Zn)", JLabel.CENTER), cc.xy(5, 15,
					"f,d"));
			builder.add(createRLButton(false, AtlParameter.ZINK_ID), cc.xy(7,
					15));
			...
			builder.add(new JLabel("<  Einheit  >", JLabel.CENTER), cc.xy(5,
					21, "f,d"));
			builder.add(getRightEinheitenBox(), cc.xy(9, 21, "c,d"));

			builder.add(getLeftAnalyseFeld(), cc.xy(1, 23, "c,d"));
			builder.add(new JLabel("<  Analyse von  >", JLabel.CENTER), cc.xy(
					5, 23, "f,d"));
	...
	private JComboBox getLeftEinheitenBox() {
		if (leftEinheitenBox == null) {
			...
			leftEinheitenBox.setSelectedItem(AtlEinheiten
	...
	private JComboBox getRightEinheitenBox() {
		if (rightEinheitenBox == null) {
			...
			rightEinheitenBox.setSelectedItem(AtlEinheiten
					.getEinheit(AtlEinheiten.MG_KG_ID));
		}
	...
	private JTextField getLeftAnalyseFeld() {
		if (leftAnalyseFeld == null) {
			...
			leftAnalyseFeld.setPreferredSize(getLeftEinheitenBox()
					.getPreferredSize());
		}
	...
	private JTextField getRightAnalyseFeld() {
		if (rightAnalyseFeld == null) {
			...
			rightAnalyseFeld.setPreferredSize(getRightEinheitenBox()
					.getPreferredSize());
		}
