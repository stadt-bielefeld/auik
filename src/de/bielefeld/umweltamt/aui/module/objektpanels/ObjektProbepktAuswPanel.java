...
public class ObjektProbepktAuswPanel extends JPanel {
	...
	public ObjektProbepktAuswPanel(ObjektBearbeiten hauptModul) {
		...
		FormLayout layout = new FormLayout (
				"l:p:g, 10dlu:g(0.3), r:16px, 5dlu, c:70dlu:g(0.1), 5dlu, l:16px, 10dlu:g(0.3), r:p:g", // Spalten
				"pref, 3dlu" +", " +  //1
				"pref, 3dlu " +", " +  //2
				"pref, 3dlu" +", " +	//3
				"pref, 3dlu" +", " +	//4
		...
		builder.add(getSubmitButton() ,        cc.xy(5, 19));
	}
	...
	private TimeSeriesCollection createDataset(String axis) {
		...
		createSeries(paramList, pkt , col);
		...
		if (pkt != null) {
			...
			for (int i = 0; i < paramList.getModel().getSize(); i++) {
				...
				TimeSeries series = ChartDataSets
						.createAnalysePositionenSeries(list, p+ " ",einheit);
